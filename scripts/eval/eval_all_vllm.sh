...
for model_path in "$PATH_TO_STAGE_ONE_MODEL" "$PATH_TO_STAGE_TWO_MODEL"; do
    if [[ "$model_path" == /* ]] || [[ "$model_path" == ./* ]]; then
        ...
        echo "Note: Using HuggingFace model ID: $model_path (will be downloaded if not cached)"
    fi
done
...
for task in "${TASKS[@]}"; do    
    for model in "${MODELS[@]}"; do
        ...
        # Run the evaluation
        lm_eval --model vllm \
            --model_args pretrained=${model},tokenizer=${model},tensor_parallel_size=4,max_gen_toks=31768,max_model_len=32768,enforce_eager=True \
            --tasks "$task" \
            --apply_chat_template \
